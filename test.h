...
#define TEST_ASSERT_EQUAL_EX(EXPECTED, GOT, CAST_TO, PRINT_TYPE) if ((CAST_TO)EXPECTED != (CAST_TO)GOT){ test_status[test_index] = FAIL; \
                                               printf( "FAILED: %s:%d %s "  #EXPECTED " = " PRINT_TYPE " != " #GOT " = " PRINT_TYPE " %s\n",\
...
#define TEST_ASSERT_EQUAL_STR(EXPECTED, GOT)  if (strcmp(EXPECTED, GOT) != 0){ test_status[test_index] = FAIL; \
                                               printf( "FAILED: %s:%d %s "  #EXPECTED " = \"%s\" != " #GOT " = \"%s\" %s\n",\
